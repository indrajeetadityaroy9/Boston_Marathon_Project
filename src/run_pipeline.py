...
this file formats and displays it. Execute: python src/run_pipeline.py
"""
import sys
import time
...
def run_rq1():
    ...
    print("\n--- Model Coefficients ---")
    for label, pairs, intercept in rq1.get_coefficients(models):
        print(f"\n  {label}:")
        for feat, coef in pairs:
            print(f"    {feat:>20}: {coef:>10.2f}")
        print(f"    {'intercept':>20}: {intercept:>10.2f}")
...
def run_rq2():
    ...
    mdls = rq2.fit_mixed_models(df)
    m0, m1r, m2r = mdls['m0'], mdls['m1_reml'], mdls['m2_reml']
    print(f"  OLS R^2={m0.rsquared:.4f}")
    print(f"  RI converged={mdls['m1_ml']['converged']}, RI+RS converged={mdls['m2_ml']['converged']}")
    ...
    for var in exog.columns:
        ...
        print(f"  {var:>10} {m0.params[var]:>14.2f} {m1r['fe_params'][lk]:>14.2f} {m2r['fe_params'][lk]:>14.2f}")

    tau2_0, tau2_1, corr_01 = m2r['tau2_0'], m2r['tau2_1'], m2r['corr_01']
    print(f"\n--- Variance Components (RI+RS) ---")
    ...
    print(f"  LRT OLS vs RI: LR={lrt01['lr_stat']:.2f}, p_corr={lrt01['p_corrected']:.2e}")
    print(f"  LRT RI vs RI+RS: LR={lrt12['lr_stat']:.2f}, p_corr={lrt12['p_corrected']:.2e}")
    best_aic = min(r[3] for r in rows)
    for name, k, llf, aic, bic in rows:
        print(f"  {name:>8} k={k} AIC={aic:,.1f} BIC={bic:,.1f} (dAIC={aic-best_aic:+.1f})")

    print("\nSTEP 7: VARIANCE DECOMPOSITION")
    vd = metrics.variance_decomposition(m2r, df, m2r_fe)
    for comp, val in [('Fixed', vd['var_fixed']), ('Intercepts', vd['tau2_0']),
                      ('Slopes', vd['var_slope']), ('Residual', vd['sigma2'])]:
        print(f"  {comp:<15} {val:>14,.0f} ({val/vd['total']:>5.1%})")
    ...
    print(f"  Marginal RMSE={pe['marg_rmse']:.1f}, Conditional RMSE={pe['cond_rmse']:.1f}, "
          f"VoP={pe['personalization']:.0f}s ({pe['personalization']/60:.1f} min)")
    print(f"  90% PI: Known={pe['pi_known']:.0f}s ({pe['pi_known']/60:.0f} min), "
          f"New={pe['pi_new']:.0f}s ({pe['pi_new']/60:.0f} min)")

    print("\nSTEP 12: TEMPORAL HOLD-OUT")
    ho = rq2.temporal_holdout(df)
    print(f"  Train: {ho['n_train']:,} obs, {ho['n_train_runners']:,} runners")
    print(f"  Known: cond RMSE={ho['cond_rmse_known']:.1f}, marg RMSE={ho['marg_rmse_known']:.1f}")
    print(f"  New: marg RMSE={ho['marg_rmse_new']:.1f}")
    print(f"  Value of personalization (OOS): {ho['marg_rmse_known'] - ho['cond_rmse_known']:.1f}s")
    print(f"  Exported {ho['n_blups_exported']:,} leak-free BLUPs")

    print("\nSTEP 13: WEATHER-AUGMENTED MODEL (temp + humidity + wind)")
    w = rq2.sensitivity_weather(df)
    print(f"  temp_c:  {w['temp_coef']:>8.1f} sec/deg C (SE={w['temp_se']:.1f}, p={w['temp_p']:.2e})")
    print(f"  humid_c: {w['humid_coef']:>8.2f} sec/%     (SE={w['humid_se']:.2f}, p={w['humid_p']:.2e})")
    print(f"  wind_c:  {w['wind_coef']:>8.1f} sec/mph   (SE={w['wind_se']:.1f}, p={w['wind_p']:.2e})")
    print(f"  Flagged years: {w['flagged']}/20, Year-level std: {w['year_std']:.1f}s, RMSE: {w['cond_rmse']:.1f}s")

    print("\nSTEP 14: LOG-TRANSFORM SENSITIVITY")
    lg = rq2.sensitivity_log(df)
    print(f"  Log residuals: skew={lg['skewness']:.4f}, kurt={lg['kurtosis']:.4f}")
    print(f"  Back-transformed RMSE: {lg['rmse_bt']:.1f}s, Smearing: {lg['smearing_factor']:.4f}")

    print("\nSTEP 15: SPLINE AGE COMPARISON")
    sp = rq2.sensitivity_spline(df)
    print(f"  Quadratic AIC={sp['quad_aic']:.1f}, Spline AIC={sp['spline_aic']:.1f}, dAIC={sp['daic']:+.1f}")
    ...
    print(f"  Active (n={sb['n_active']:,}): {sb['slope_active']:.1f} sec/yr")
    print(f"  Dropped (n={sb['n_dropped']:,}): {sb['slope_dropped']:.1f} sec/yr")
    print(f"  Diff: {sb['diff']:.1f} sec/yr ({sb['pct_diff']:.1f}%)")
...
def run_rq3():
    ...
    n_blup = test['blup_intercept'].notna().sum()
    print(f"  Runners with complete splits: {len(train) + len(test):,}")
    print(f"  Leak-free BLUPs: {n_blup:,} ({n_blup/(len(train)+len(test))*100:.1f}%)")
    ...
    print(f"\n{'CP':>10} {'Naive':>8} {'Splits':>8} {'Demo':>8}")
    print("-" * 38)
    for cp in cfg.CP_ORDER:
        print(f"  {cp:>8} {pivot.loc[cp, cfg.NAIVE]:>6.0f} {pivot.loc[cp, cfg.SPLITS]:>6.0f} {pivot.loc[cp, cfg.DEMO]:>6.0f}")

    blup_r = results_df[results_df['model'].isin([cfg.FULL, cfg.SPLITS_SUBSET])]
    print(f"\n--- History Augmentation (n={blup_r['n_test'].iloc[0]:,}) ---")
    for cp in cfg.CP_ORDER:
        ...
        print(f"  {cp:>8} {sr:>6.0f} {fr:>6.0f} {sr - fr:>+6.0f}")
    ...
    print(f"  Personalized RMSE benchmark: {cfg.PERSONALIZED_RMSE:.0f}s")
    for _, r in xover.iterrows():
        marker = " <-- CROSSOVER" if r['is_crossover'] else ""
        print(f"  {r['checkpoint']:>8} {r['rmse']:>6.0f}s {'YES' if r['beats'] else 'no':>6}{marker}")

    print("\nSTEP 4b: CUMULATIVE vs SINGLE")
    cvs, lo, hi = rq3.cumulative_vs_single(results_df)
    for _, r in cvs.iterrows():
        print(f"  {r['checkpoint']:>8} {r['cumul_rmse']:>6.0f} {r['single_rmse']:>6.0f} {r['advantage']:>+6.0f}")
    print(f"  Mid-race advantage: {lo:.0f} to {hi:.0f} seconds")

    print("\nSTEP 4c: YEAR DEGRADATION")
    yd, max_d = rq3.year_degradation(results_df)
    for _, r in yd.iterrows():
        print(f"  {r['checkpoint']:>8} {r['no_year']:>6.0f} {r['with_year']:>6.0f} {r['degradation']:>+6.0f}")
    print(f"  Maximum degradation: {max_d:.0f} seconds")

    print("\n--- 90% PI Width Convergence ---")
    for _, r in rq3.pi_convergence(results_df).iterrows():
        print(f"  {r['checkpoint']:>8} RMSE={r['rmse']:>6.0f} PI={r['pi_width']:>6.0f}s ({r['pi_min']:.1f} min)")
