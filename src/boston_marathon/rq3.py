...
def _fit_ridge(X_tr, y_tr, X_te, y_te):
    """Train a Ridge model, predict on test data, and return accuracy metrics."""
...
def run_progressive(train, test):
    ...
    for i, cp in enumerate(cfg.SPLIT_COLS):
        ...
        # Naive: if you're running 5K pace, assume that pace holds for 42.195 km
        add(cfg.NAIVE, regression_metrics(y_te, test[cp].values * (cfg.MARATHON_KM / km)), len(test))

        # Splits: use all checkpoint times so far, plus compute the prediction interval
...
def quantile_pi(train, test):
    ...
    from boston_marathon.metrics import empirical_coverage
    ...
    for i, cp in enumerate(cfg.SPLIT_COLS):
        ...
        train_resid = y_tr - m.predict(X_tr)
        pctl_lo = preds + np.percentile(train_resid, 5)
        pctl_hi = preds + np.percentile(train_resid, 95)

        rows.append({
            'checkpoint': cfg.CP_ORDER[i],
            'qr_width': np.mean(hi - lo), 'qr_coverage': empirical_coverage(y_te, lo, hi),
            'pctl_width': np.mean(pctl_hi - pctl_lo), 'pctl_coverage': empirical_coverage(y_te, pctl_lo, pctl_hi),
        })
