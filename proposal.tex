...
\begin{enumerate}
    ...
    \item \textbf{Prediction:} use machine learning models on checkpoint split data to predict finish times, serving as a secondary validation.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Q3 (Predictive Validation):} How accurately can we predict a runner's finish time from their checkpoint splits? Does a simple linear model suffice, or do nonlinear models perform better?

    Q1 and Q2 are about understanding---they explain which factors are associated with finish time and how large the effects are. Q3 asks a different question: can we actually predict a runner's finish time using the patterns we find? This serves two purposes. First, it tests whether the relationships discovered in Q1 and Q2 are strong enough to be useful for prediction, not just statistically significant. Second, comparing linear models (ridge, LASSO) against a nonlinear model (gradient boosting) reveals whether the relationship between checkpoint splits and finish time is a simple straight line or involves interactions and curves that the linear models miss.
\end{itemize}
...
\subsection{Q3: Predictive Models}

Q1 and Q2 focus on inference---understanding which factors relate to finish time and estimating their effects. Q3 asks a complementary question: can these patterns actually predict a runner's finish time? This serves as a reality check on the earlier findings. If age and gender explain meaningful variation (as Q1 and Q2 suggest), then models using these features plus mid-race checkpoint times should predict finish times accurately. If they cannot, it suggests the statistical associations found in Q1 and Q2 are too weak or too noisy to be practically useful.

Comparing a linear model (ridge/LASSO) against a nonlinear model (gradient boosting) also reveals whether the relationship between split times and finish time is a simple straight line or involves interactions (e.g., whether slowing down after the half depends on how fast the first half is). If the nonlinear model performs much better, it indicates that the linear assumptions behind ridge and LASSO are too simple for this data.

We train three models on the 2015--2017 split-time data (79,038 rows) to predict finish time from the nine checkpoint times and demographic features (age, gender):

\begin{itemize}
    \item \textbf{Ridge regression} is ordinary least squares with an added penalty that shrinks all coefficients toward zero. The penalty is proportional to the sum of squared coefficients. This stabilizes the estimates when predictors are highly correlated (as the checkpoint times are, with $r > 0.99$ between adjacent splits). Without this penalty, the coefficients are large and unstable because the model cannot distinguish the contribution of one checkpoint from the next.

    \item \textbf{LASSO regression} uses a similar penalty, but proportional to the sum of absolute values of coefficients instead of squared values. This has the additional property of shrinking some coefficients to exactly zero, effectively performing automatic feature selection. It is useful for determining which checkpoints are most informative.

    \item \textbf{Gradient boosting} builds an ensemble of small decision trees, where each tree corrects the errors of the previous ones. Unlike ridge and LASSO, it can capture nonlinear relationships---for example, if the effect of a slow first 5k on finish time depends on whether the runner speeds up or slows down later.
\end{itemize}
